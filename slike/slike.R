...
prvi_graf <- function() {
  ...
  barplot(apply(as.matrix(prvih10[ ,c(7,9)]), 1, c), beside = TRUE, xlab = "Countries", width = 1, 
          ...
          main = "Delež populacije z internetom v primerjavi z dele?em dr?ave v svetovnih uporabnikih interneta.",
          col = c("grey", "grey1"))
...
drugi_graf <- function() {
  barplot(as.matrix(t7), beside = TRUE, ylim = c(0, 200), col = c(brewer.pal(9, "Blues"),"black"), 
          main = "?tevilo dr?av razvr???enih v skupine glede na dele? uporabnikov interneta\n po posameznih letih", 
          names.arg = substr(names(t7), 2, 5))
  legend("topright", 
...
tretji_graf <- function() {
  ...
  print(qplot(rownames(dr), dr$Penetration....of.population.with.Internet.,
              ...
              main = "Dele? uporabnikov interneta med 2000-2014")
        + theme(axis.text.x = element_text(size = 5)))
}

# Pita iz tabele t4
cetrti_graf <- function() {
  pie3D(t4[1:7, 5], labels = rownames(t4)[1:7], radius = 0.9, explode = 0.2, main = "Dele? uporabnikov glede na geografsko regijo",
