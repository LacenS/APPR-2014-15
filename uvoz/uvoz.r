...
#1 tabela v obliki csv iz te strani: http://data.worldbank.org/indicator/IT.NET.USER.P2
uvoz_tabele1 <- function() {
  t1 <- read.csv("podatki/uporabniki_interneta_po_drzavah.csv", skip = 2, na.strings = "NA", 
                 sep = ",", dec=".", fileEncoding="Windows-1252")
  tabela_1<-t1[, c(1, 3, 44:58)] #podatkov iz drugih stolpcev ne potrebujem
  #Številske podatke pretvorimo v "števila" in jih zaradi preglednosti zaokrožimo
  ...
  stolpci <- gsub("[X]", "", colnames(tab1)) # V kasnejši fazi se je ta korak izkazal za slabo potezo
...
cat("Uvažam podatke o uporabnikih interneta po posameznih državah v letih 1990-2013.\n")


#2. uvoz podatkov iz spleta iz te strani: http://www.internetlivestats.com/internet-users-by-country/
...
uvoz_tabele3 <- function(){
  ...
  tables <- readHTMLTable(u)
  tabela3 <- tables[2] # tabela, ki jo želim je drugi element na tej spletni strani
...
cat('Uvažam podatke o deležu uporabnikov interneta glede na celotno populacijo med leti 1993-2014.\n')
...
cat("Uvažam podatke o uporabnikih interneta po geografskih regijah-kontinentih.\n")
...
#izkazala se je za neuporabno saj so ti podatki tudi v zemljevidu svet
...
#tabela sestavljena iz t1 in t2, ki jo bom uporabila pri kasnejši analizi
#Verjetno obstaja lažji način...:)
tabela_7 <- function(){
